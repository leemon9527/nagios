...
def get_raidstate(raidstate,index="ALL"):
    ...
    if index=="ALL":
        for k,v in raidstate.items():
            if v != "optimal, enabled":
                flag="Critical; "
                msg= "Raid {} : {}".format(k,v)
                message.append(msg)
    else:
        if index < len(raidstate):
            if raidstate[index]!="optimal, enabled":
                flag="Critical; "
...
def init(finame):
    ...
    f = open(filename,'r')
    for line in f:
        if "PhysDisk State:" in line:
            tmpstate=line.strip().split(':')[1].strip()
            diskstate[i]=tmpstate
            i+=1
        if re.match('Volume \d State:',line):
        # if "Volume 0 State:" in line:
            tmpstate=line.strip().split(':')[1].strip()
